...
| Step | Description | 说明 |
|:-----|:------------|:-----|
| 1 | Copy workflows (`/loop`, `/loop-issue`, `/loop-status`) into `.agents/workflows/` | 安装工作流文件到 `.agents/workflows/` |
...
| **Step 1** | Check open PRs, fix CI failures, report status (human merges) | 检查已有 PR，修复 CI 失败，报告状态（人来合并） |
...
| **Step 4** | Run build & tests, create PR with `Closes #N`, **loop back if more issues** | 运行构建和测试，创建关联 PR，**有更多 Issue 则继续** |
| **Step 5** | Append session summary (all issues) to `.agents/loop-history.md` | 将本轮所有 Issue 摘要追加到历史记录文件 |
