...
3. **Sub-task size check / 子任务规模检查**: Before starting, assess the sub-task scope. If a single sub-task would touch >15 files or require >3 distinct feature areas, report to user: "Sub-task M is large (estimated N files). Consider splitting it in the plan before implementation." Proceed only if user confirms, or if the sub-task is clearly atomic despite its size. / 如果单个子任务预计修改 >15 个文件或涉及 >3 个功能区域，向用户报告并建议拆分。
4. Create branch: `git checkout -b issue-<N>-subtask-<M>-<description>`
5. Implement following Direct Implementation flow.
6. Update `docs/plans/<topic>.md`: mark completed sub-task `- [x]` with PR number.
7. Proceed to Step 4. **Use Sub-task PR body template**.
...
1. **Run verification** (from `AGENTS.md`): build, test, lint, type-check.
   - If a verification command fails because the project setup has fundamentally changed (e.g., test runner was removed, build tool was replaced), do NOT treat this as a regression. Instead:
     - Note it in the PR's "What I Did NOT Test" section
     - Add a comment in the PR: "⚠️ AGENTS.md verification commands may need updating after this change"
     - This prevents false-negative failures when sub-tasks intentionally transform the project (e.g., scaffolding replaces the old test framework)
   - 如果验证命令因项目架构根本性变更而失败，不要当作回归问题。在 PR 中注明，并提醒更新 AGENTS.md。
...
- **AGENTS.md sync check / AGENTS.md 同步检查**: After implementation, if the PR changes `package.json` dependencies, build tools, or test frameworks, add a note in the PR body: "⚠️ Tech stack changed — `AGENTS.md` may need updating. Consider running `/loop-init` to regenerate." / 如果 PR 更改了依赖、构建工具或测试框架，在 PR 中提醒更新 AGENTS.md。
- **Scaffold lint tolerance / 脚手架 Lint 容忍**: When scaffolding new projects (e.g., `create-vite`, `create-next-app`), the default template code may have lint warnings. These are acceptable in the scaffolding PR and should be listed in "What I Did NOT Test" with a note that they will be resolved in subsequent sub-tasks. / 使用脚手架创建新项目时，默认模板代码可能有 lint 警告，这在脚手架 PR 中是可接受的。
